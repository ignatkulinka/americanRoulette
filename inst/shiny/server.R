...
function(input, output) {
  ...
  output$mainPlot <- renderPlot({
    # [,1]: balance [,2]: count
    graph <- ggplot(data = NULL, aes(x = outcomesList$data[, 2], y = outcomesList$data[, 1])) +
      geom_line() +
      # labs(title = 'Total Balance vs. Number of Bets') +
      geom_point() +
      ggthemes::theme_hc() +
      labs(x = "Number of Rounds", y = "Total Balance") +
      xlim(0, length(outcomesList$data) + 5) +
      coord_cartesian(ylim = c(min(outcomesList$data[, 1]) - 5, max(outcomesList$data[, 1]) + 5))
    graph
  })
  ...
  output$leftPlot <- renderPlot({
    ...
    graph <- ggplot(data = df, aes(x = names, y = num)) +
